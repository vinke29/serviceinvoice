...
function AIAgent() {
    ...
    const handleFirebaseConfig = (fbConfig) => {
      ...
      if (fbConfig) {
        ...
        const mappedConfig = {
          ...
          netDays: parseInt(fbConfig.netDays || 14, 10),
  ...
  // Save config to Firestore whenever it changes (debounced)
  ...
  // Inline edit handlers (update state immediately)
  const handleInlineSave = (updates) => {
    ...
    setAgentConfigState(newConfig); // This will trigger the debounced save
  };
  ...
  if (saving) {
    return <div className="min-h-[300px] flex items-center justify-center text-lg text-secondary-600">Saving agent config...</div>;
  }

  if (saveError) {
    return <div className="min-h-[300px] flex items-center justify-center text-lg text-red-600">{saveError}</div>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {!agentConfig && (
        <OnboardingConfig onSave={handleInlineSave} initialConfig={agentConfig} />
      )}
